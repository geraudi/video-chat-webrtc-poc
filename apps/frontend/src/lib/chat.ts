...
export function setLocalVideo(video: HTMLVideoElement) {
  localVideo = video;
}
...
async function handleNegotiationNeededEvent() {
  ...
  if (!myPeerConnection || !strangerId) return;
  ...
  try {
    ...
    if (role === 'callee') {
      // Insure caller receive video answer
      // prevent error DOMException: Cannot set remote offer or answer in current state have-remote-offer
      // because caller receive our VIDEO OFFER before receive VIDEO ANSWER
      // so wait before sending VIDEO OFFER.
      // Try to use SQS with API Gateway and Lambda to handle this issue
      // (https://scribe.rip/building-resilient-webhooks-with-aws-api-gateway-and-sqs-integration-6bbb398cc448)
      await sleep(1000);
    }
...
async function handleVideoOfferMsg(msg: VideoOfferInputMessage) {
  ...
  // todo remove from here. Init cam on connect
  if (!webcamStream) {
    try {
      webcamStream =
        await navigator.mediaDevices.getUserMedia(mediaConstraints);
